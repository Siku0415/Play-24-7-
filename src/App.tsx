...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] font-sans text-gray-900 overflow-x-hidden selection:bg-indigo-100 italic-none">
      {/* Header */}
      <header className="bg-white px-4 py-4 flex items-center justify-between sticky top-0 z-[100] shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-black text-[#051139] tracking-tight">Play24x7</span>
        </div>
        ...
        <nav className="flex gap-2">
          {["Home", "Rates", "Games"].map((tab) => (
            <a 
              ...
              className="bg-[#eef2ff] text-[#1a237e] px-4 py-2 rounded-full text-xs font-[1000] shadow-sm whitespace-nowrap active:scale-95 transition-transform"
            >
              {tab}
            </a>
          ))}
      ...
      <section id="hero" className="bg-white pt-12 pb-14 px-4 relative">
        <div className="max-w-md mx-auto text-center">
          <h1 className="text-[2.75rem] font-[1000] text-[#051139] leading-tight mb-2 tracking-tight">
          ...
          <div className="mb-12">
            <h2 className="text-5xl font-[1000] text-[#051139] leading-[1] uppercase tracking-tighter italic mb-6">
              PLAY BIG <br />
              <span className="text-red-600 block mt-2 text-6xl">WIN BIG</span>
            </h2>
            <div className="bg-yellow-400 text-[#051139] font-[1000] py-4 rounded-2xl shadow-xl mb-4 inline-block px-8 transform -rotate-1 border-2 border-[#051139]">
          ...
          <div className="px-2">
            <a 
              href={downloadUrl}
              className="flex items-center justify-center gap-3 w-full bg-[#051139] text-white py-5 rounded-full font-black text-xl shadow-2xl hover:brightness-110 active:scale-95 transition-all outline outline-offset-2 outline-indigo-200"
            >
      ...
      <section id="pricing" className="py-16 bg-[#f1f5f9] px-4">
        <div className="max-w-md mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-7xl font-[1000] text-[#051139] mb-4 tracking-tighter">Game Rates</h2>
          ...
          <div className="bg-slate-200 p-1 rounded-xl flex mb-10">
            <button 
              ...
              className={`flex-1 py-4 rounded-lg font-black text-sm transition-all ${activeRateTab === 'main' ? 'bg-[#051139] text-white shadow-lg' : 'text-slate-600'}`}
            >
              Main Market
            </button>
            <button 
              onClick={() => setActiveRateTab("gali")}
              className={`flex-1 py-4 rounded-lg font-black text-sm transition-all ${activeRateTab === 'gali' ? 'bg-[#051139] text-white shadow-lg' : 'text-slate-600'}`}
            >
              Galidesawar
            </button>
          </div>
          ...
          <div className="space-y-3">
            {rates[activeRateTab as keyof typeof rates].map((item) => (
              <div key={item.id} className="bg-white border-l-[10px] border-[#051139] flex items-center p-5 rounded-xl shadow-sm">
                <div className="w-12 h-12 flex items-center justify-center bg-slate-50 rounded-full shrink-0">
                  <ThumbsUp className="w-6 h-6 text-[#051139]" />
                </div>
                <div className="flex-1 font-black text-xl ml-4 text-slate-800">{item.name}</div>
                <div className="font-black text-xl text-[#051139]">{item.rate}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="availableGames" className="py-20 bg-white px-4">
        <div className="max-w-md mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-[5rem] font-[1000] text-[#051139] leading-[0.8] tracking-tighter mb-4">
          ...
          <div className="bg-slate-200 p-1 rounded-xl flex mb-12">
            <button 
              ...
              className={`flex-1 py-4 rounded-lg font-black text-sm transition-all ${activeGameTab === 'main' ? 'bg-[#051139] text-white shadow-lg' : 'text-slate-600'}`}
            >
              Main Market
            </button>
            <button 
              onClick={() => setActiveGameTab("gali")}
              className={`flex-1 py-4 rounded-lg font-black text-sm transition-all ${activeGameTab === 'gali' ? 'bg-[#051139] text-white shadow-lg' : 'text-slate-600'}`}
            >
              Galidesawar
            </button>
          </div>
          ...
          <div className="space-y-4">
            {games.map((game, index) => (
              <div key={index} className="bg-white border-l-[10px] border-[#051139] rounded-3xl flex items-center py-8 px-6 shadow-md relative border border-gray-100 group">
                <div className="flex-1 text-center pr-2">
                  <h4 className="text-2xl font-[1000] text-[#051139] mb-2 uppercase tracking-tighter italic">
                    {game.name}
                  </h4>
                ...
                <div className="flex flex-col items-center shrink-0">
                  <a href={downloadUrl} className="flex flex-col items-center group/btn">
                    <div className="w-16 h-16 bg-[#051139] rounded-full flex items-center justify-center shadow-lg transition-transform group-hover/btn:scale-110">
      ...
      {/* Footer */}
      <footer className="bg-[#051139] text-white pt-20 pb-16 px-6 flex flex-col items-center text-center">
        ...
        <p className="text-gray-400 text-xs font-black uppercase tracking-[0.2em] leading-relaxed">
